...
export const scheduleCron = async () => {
    ...
    // Cron /data_xml/h2h_data.xml 1 per day 4 phút
    schedule.scheduleJob("cron-xmlH2h-4-minutes", "*/4 * * * *", async () => {
        await xml_h2h();
        console.log("Crawling odds h2h every 4 minutes...");
    });

    // Cron /data_xml/scheduleAll_data.xml 1 per day 6 phút
    schedule.scheduleJob("cron-xmlSchedule-5-minutes", "*/6 * * * *", async () => {
        await xml_schedule();
        console.log("Crawling schedule XML sau 5 phút ...");
    });

    // Cron xml_3in1_3Day every day 00: 08
    schedule.scheduleJob("cron-xml3in13Day-30-minutes", "*/30 * * * *", async () => {
        await xml_3in1_3Day();
    ...
    schedule.scheduleJob("cron-xmlH2h-3-day-20-minutes", "12 0 * * *", async () => {
        await xml_h2h_3day();
        console.log("Crawling odds h2h every 20 minutes...");
    });

    // Deleted xml_h2h_3day every day 00:20
    // schedule.scheduleJob("cron-xml_schedule_deleted-20-minutes", "20 0 * * *", async () => {
    ...
    schedule.scheduleJob("cron-updateScheduleFor3Days-daily", "4 0 * * *", async () => {
    ...
    schedule.scheduleJob("cron-deleteScheduledMatchesForDate-daily", "*2 0 * * *", async () => {
