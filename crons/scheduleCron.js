...
export const scheduleCron = async () => {
    ...
    schedule.scheduleJob("cron-xmlOddsChangeDetail-2-minutes", "*/3 * * * *", async () => {
        await xml_3in1();
    ...
    schedule.scheduleJob("cron-xmlSchedule3Day-daily", "*/10 * * * *", async () => {
        await xml_schedule3Day();
        console.log("Crawling xml_schedule3Day sau cron daily...");
    });

    schedule.scheduleJob("cron-deleteScheduledMatchesForDate-daily", "*/11 * * * *", async () => {
        await deleteScheduledMatchesForDate();
        console.log("Crawling deleteScheduledMatchesForDate sau cron daily...");
    });
    schedule.scheduleJob("cron-updateScheduleFor3Days-daily", "*/12 * * * *", async () => {
    ...
    schedule.scheduleJob("cron-xml_odds_3Day-30-minutes", "*/14 * * * *", async () => {
        await xml_odds_3Day();
        console.log("Crawling xml_odds_3Day sau cron daily...");
    });

    schedule.scheduleJob("cron-xml3in13Day-30-minutes", "*/16 * * * *", async () => {
        await xml_3in1_3Day();
